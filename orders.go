...
type ModifyOrder structs.Order
...
func (client *FtxClient) ModifyTriggerOrder(orderId int64, triggerPrice float64, price float64) (ModifyOrder, error) {
	var order ModifyOrder
	requestBody, err := json.Marshal(map[string]float64{"triggerPrice": triggerPrice, "orderPrice": price})
	if err != nil {
		log.Fatal(err)
		return order, err
	}
	resp, err := client._post("conditional_orders/"+strconv.FormatInt(orderId, 10)+"/modify", requestBody)
	if err != nil {
		log.Fatal(err)
		return order, err
	}
	err = _processResponse(resp, &order)
	return order, err
}

func (client *FtxClient) ModifyOrder(orderId int64, price float64) (ModifyOrder, error) {
	var order ModifyOrder
	requestBody, err := json.Marshal(map[string]float64{"price": price})
	if err != nil {
		log.Fatal(err)
		return order, err
	}
	resp, err := client._post("orders/"+strconv.FormatInt(orderId, 10)+"/modify", requestBody)
	if err != nil {
		log.Fatal(err)
		return order, err
	}
